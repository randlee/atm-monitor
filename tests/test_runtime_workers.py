...
class P: repo_timeout_seconds=0.2
def fixture(repo, root, policy, now, agents, shadow):
 if repo['slug']=='r28': time.sleep(10)
...
class Workers(unittest.TestCase):
 def test_all_repos_return_and_hung_is_bounded(self):
  ...
  got=list(results(repos,'/tmp',P(),0,(),worker_limit=4,timeout=.2,target=fixture))
  self.assertLess(time.monotonic()-start,5); self.assertEqual(len(got),30)
